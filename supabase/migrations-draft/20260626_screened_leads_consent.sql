...
-- Apply only after:
--   (a) the widget consent checkbox is live for at least one firm, AND
--   (b) the operator has confirmed which firm to enable first.
-- After applying, run the implied-consent backfill (see runbook) to stamp
-- existing rows as 'implied' before enabling the gate for any firm.
--
