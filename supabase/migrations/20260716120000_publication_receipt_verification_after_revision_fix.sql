...
-- before this migration is ever applied to production). All 18
-- pre-existing checks from the two earlier hardening migrations are
-- re-verified unaffected; the new checks specific to this fix are:
--   19. THE BUG, reproduced: publish an approved deliverable at v1, create
--       an unverified receipt bound to v1, post a new version v2 (which
--       resets approved_version_id to null and status to 'in_review')
--       WITHOUT this fix applied -- verifying the v1 receipt is rejected
--       with the version-drift or approved-version-mismatch exception.
--   20. THE FIX: same setup, WITH this fix applied -- verifying the v1
--       receipt (verification_state='verified', reconciles_receipt_id set
--       to the original, approved_version_id copied unchanged from the
--       original at v1) succeeds even though the deliverable is currently
--       'in_review' with approved_version_id null.
--   21. A verification/failure/reconciling row that reconciles a real
--       receipt but supplies a DIFFERENT approved_version_id than the one
--       it reconciles -- rejected (the new additive check; proves the
--       exemption cannot be used to smuggle a different version).
--   22. A fresh 'unverified' receipt with reconciles_receipt_id set (the
--       general correction path, unused by any caller today) against a
--       deliverable NOT currently approved -- still rejected (proves the
--       exemption is scoped to verified/failed/reconciling only).
--   23. A first-ever receipt inserted directly with
--       verification_state='verified' and reconciles_receipt_id null,
--       against a deliverable not currently approved -- still rejected
--       (proves a row with no prior receipt to reconcile stays on the
--       strict path regardless of its own verification_state).
-- ---------------------------------------------------------------------------
