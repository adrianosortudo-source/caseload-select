...
/**
 * Must match the append-only geography table check constraint. The public
 * evidence package deliberately has one accepted identifier so the operator
 * adapter can pass it through without aliasing or weakening the evidence gate.
 */
export const DOWNTOWN_PLAN_41_BOUNDARY_ID = "toronto-official-plan-secondary-plan-41" as const;
...
export type ProspectDowntownGeographyObservation = Readonly<{ observationId: string; evidenceIds: readonly string[]; geography: Readonly<{ sourceRecordKey: string; boundaryId: typeof DOWNTOWN_PLAN_41_BOUNDARY_ID; status: "inside" | "outside" | "needs_manual_review"; normalizedAddress: string; latitude: number | null; longitude: number | null; coordinateSourceType: "toronto_one_address_repository" | "reviewed_geocoder" | "reviewed_firm_website" | "manual_review" | null; coordinateSourceUrl: string | null; boundarySourceUrl: string; boundaryGeometrySha256: string; observedOn: string; confidence: "high" | "moderate" | "unknown"; note: string | null }> }>;
...
function parseGeo(value: unknown, ids: ReadonlySet<string>, known: ReadonlySet<string> | undefined, issues: GtaProspectEvidenceImportIssue[]): ProspectDowntownGeographyObservation[] {
  ...
  for (const [i, item] of value.entries()) {
    ...
    if (geo.boundaryId !== DOWNTOWN_PLAN_41_BOUNDARY_ID) issues.push({ path: `${path}.geography.boundaryId`, message: "must be the canonical Toronto Downtown Plan (Secondary Plan 41) boundary ID" }); if (!["inside", "outside", "needs_manual_review"].includes(geo.status as string)) issues.push({ path: `${path}.geography.status`, message: "is invalid" });
