...
export function researchFirmHeading(detail: ProspectEnrichmentFirmDetail) {
  ...
  return { stableIds, domains, identityState: identity?.state ?? "error", latestObservation: observations[0]?.dateLabel ?? "Observation date not recorded", freshness: observations.some((item) => item.freshness === "refresh_recommended") ? "Refresh recommended for older evidence" : observations.some((item) => item.freshness === "current") ? "Dated evidence is within its display refresh period" : "Evidence freshness is unknown", complete: detail.complete };
}
