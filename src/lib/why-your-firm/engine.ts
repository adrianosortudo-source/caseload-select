...
/* ──────────────────────────────────────────────────────────────────
 *  Custom claims · the lawyer's own words, one per category
 * ────────────────────────────────────────────────────────────────── */

/**
 * A custom claim has no deck card behind it, so it needs an id the rest of
 * the engine can carry as if it did. The id is derived from the category, one
 * custom claim per category at most, which is what makes it stable across a
 * reload and what lets capAndRank order it against the deck.
 */
...
export function customIdFor(category: Category): string {
  return `${CUSTOM_ID_PREFIX}${category}`;
}

function categoryFromCustomId(id: string): Category | null {
  const raw = id.slice(CUSTOM_ID_PREFIX.length);
  const meta = CATEGORIES.find((c) => c.id === raw);
  return meta ? meta.id : null;
}

/**
 * Turns any card id, deck or custom, into a card the engine can judge.
 *
 * A custom card is synthesised from the typed text and carries no
 * complianceFlag and no crowdFlag, because neither concept applies to a
 * sentence the deck has never seen. It does carry the same shape, so every
 * downstream consumer (judgeCard, the brief, the PDF) treats it identically.
 *
 * Returns null when the id is custom but no claim text was typed for that
 * category, and when a deck id is unknown. Callers filter nulls out rather
 * than rendering a half-built card.
 */
export function materializeCard(
  cardId: string,
  customClaims: Partial<Record<Category, string>>,
): DifferentiatorCard | null {
  if (!isCustomId(cardId)) return getCard(cardId) ?? null;

  const category = categoryFromCustomId(cardId);
  if (!category) return null;

  const claim = (customClaims[category] ?? "").trim();
  ...
  return {
    ...
    claim,
    proofPrompt: copy.custom.proofPrompt,
    inputLabel: null,
    complianceFlag: null,
    crowdFlag: false,
  };
 ...
 *
 * Custom claims rank after the whole deck, in CATEGORIES order among
 * themselves. Deck cards come first not because they are worth more but
 * because their order is the one thing two lawyers with the same selections
 * can be guaranteed to share; a custom claim has no deck position to sort by,
 * so it takes the tail and the cap applies to the combined list.
 */
export function capAndRank(cardIds: string[]): string[] {
  ...
  const customInCategoryOrder = CATEGORIES.map((c) => customIdFor(c.id)).filter((id) =>
    idSet.has(id),
  );

  return [...inDeckOrder, ...customInCategoryOrder].slice(0, KEEP_CAP);
}
...
function dedupeRules(rules: ComplianceRule[]): ComplianceRule[] {
...
/**
 * WHICH TEXT THE FILTER READS, AND WHY IT DIFFERS BY CARD KIND
 * A deck claim is pre-vetted: every one of the thirty-eight was written
 * against the rules, and the four that trip one carry the rule id in
 * complianceFlag. So for a deck card only the lawyer's own proof line is
 * unvetted text, and only it is scanned.
 *
 * A custom claim is the opposite. The lawyer authored the sentence itself,
 * so the claim is exactly as unvetted as the proof line, and both are
 * scanned. Skipping the claim here would be the whole hole: a lawyer could
 * type "we are the best firm in Toronto" as the claim, put a clean proof
 * line under it, and carry a superlative straight into the brief.
 *
 * The two texts are scanned separately rather than concatenated, so no rule
 * phrase can be conjured out of the seam between them.
 */
export function evaluateCardCompliance(
  ...
  const textRules = isCustomId(card.id)
    ? dedupeRules([...rulesTriggeredBy(card.claim), ...rulesTriggeredBy(proofText)])
    : rulesTriggeredBy(proofText);
...
export function computeSurvivors(
  ...
  const judged = work
    .map((w) => {
      const card = materializeCard(w.cardId, customClaims);
      if (!card) return null;
...
export interface BriefInput {
  ...
  firmName: string;
  /**
   * The lawyer's own claim text, at most one per category. Required, not
   * optional: a work entry can carry a custom id, and without the text behind
   * it the server would silently drop a claim the lawyer believed they made.
   * Pass an empty object when no custom claim was written.
   */
  customClaims: Partial<Record<Category, string>>;
}
...
export function assembleBrief(input: BriefInput): BriefData {
  ...
  const { survivors, dropped, survivorCategories } = computeSurvivors(
    input.work,
    input.customClaims,
  );
