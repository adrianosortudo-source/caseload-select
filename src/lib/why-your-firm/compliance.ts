...
export const copy = {
  tool: {
    ...
    /**
     * The line that ships once the assist exists. privacyNoGate above claims
     * nothing is ever sent, which stops being true the moment a lawyer asks
     * for a tighter claim, so this variant names that one exception plainly
     * instead of leaving the stronger sentence standing and false. Naming
     * the exception is the whole point: a privacy claim that overstates by
     * one clause is exactly what this tool teaches lawyers not to write.
     */
    privacyNoGateAssist:
      "Your answers stay in this browser until you clear them. If you choose Tighten this claim, only that claim and any proof line you entered are sent to Google Gemini for processing. CaseLoad Select does not store that text. Do not enter client names, matter details, or confidential information. Use your browser's print option if you want a copy to keep.",
    start: "Start",
    resumeEyebrow: "Welcome back",
  ...
  step4: {
    ...
    anchorLabel: "Which claim best backs up this sentence?",
  ...
  gate: {
    ...
    /**
     * Validation and delivery messages for the gated modes. Dormant while
     * GATE_MODE is no_gate (no form is rendered), kept here rather than as
     * literals in the component so a future public launch reviews its copy in
     * one file like everything else.
     */
    errFirstName: "Please enter your first name.",
    errFirmName: "Please enter your firm's name.",
    errEmail: "Please enter a valid email address.",
    errSend: "We could not email the PDF, but your brief is ready below.",
    errNetwork: "We could not reach the mail server, but your brief is ready below.",
  },
  ...
  /**
   * Side menu and question counter. The wizard runs one question per screen,
   * so the counter's denominator moves with the lawyer's own choices (more
   * kept claims means more test screens). That is correct behavior, not
   * drift: the tool reports the run it is actually on.
   */
  nav: {
    questionLabel: "Question",
    questionOf: "of",
    menuHeading: "All questions",
    menuOpen: "All questions",
    menuClose: "Close",
    groupAlternatives: "Real alternatives",
    groupDifferentiators: "Your differentiators",
    groupKeep: "Keep six",
    groupTests: "The tests",
    groupStatement: "Your statement",
    groupBrief: "Your brief",
    labelAlternatives: "What clients do instead",
    labelKeep: "Keep six",
    labelStatement: "Say it in one sentence",
    labelResults: "Your brief",
  },

  /**
   * The custom differentiator entry, one per category. The deck holds 38
   * claims and cannot hold every firm's edge, so each group ends with an
   * opening for the lawyer's own words. A custom claim then runs the same
   * route as a deck claim: kept or cut, tested, evidenced, and checked
   * against the same rules.
   */
  custom: {
    label: "In your own words",
    invitation:
      "If the thing that sets your firm apart is not on this list, write it here.",
    placeholder: "One sentence, the way you would say it to a client.",
    proofPrompt:
      "What would you show someone who asked you to back this up?",
    empty: "Write the claim before keeping it, or clear it to move on.",
  },

  /**
   * The assist. Advisory only: it suggests wording, the lawyer decides, and
   * the rule check runs on whichever version wins either way. The choice
   * actions reuse copy.step3.ruleKeepMine and ruleUseConversion, which were
   * written for exactly this decision.
   */
  assist: {
    button: "Tighten this claim",
    working: "Working",
    intro: "The tool suggests wording. You decide which version is yours.",
    yoursLabel: "What you wrote",
    suggestionLabel: "A tighter version",
    concernsLabel: "Rule check",
    categoryDiffers: "This may fit better under another group:",
    unavailable:
      "No suggestion came back this time. Your wording stands, and every check still runs.",
  },
