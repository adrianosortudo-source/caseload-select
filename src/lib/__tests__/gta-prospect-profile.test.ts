...
const records: ReconciledGtaProspect[] = ["selected", "held", "rejected", "incomplete"].map((status, index) => ({
...
describe("retained profile search and exact field filters", () => {
  it.each(["selected", "held", "rejected", "incomplete"])("searches facts, source URLs, dates and original %s disposition without a UUID", status => {
    const index = ["selected", "held", "rejected", "incomplete"].indexOf(status);
    for (const query of [status, "criteria-" + index, "blocked-" + index, "source-" + index]) expect(filterReconciledGtaProspects(records, { query }).map(row => row.id)).toEqual(["fixture-" + status]);
  ...
  it("does not treat supported supplemental intake as every possible website opportunity", () => {
    ...
    expect(filterReconciledGtaProspects(records, { websiteOpportunityType: "public_site_review" })).toHaveLength(4);
  });
});
