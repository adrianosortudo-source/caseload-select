 ...
 *   - waived with waiver_consent_id allows
...
// Mutable state so each test can inject a different conflict row.
const mockState = {
...
describe('checkStageGate conflict gate: waived', () => {
  it('waived WITH waiver_consent_id allows retainer_pending', async () => {
    mockState.conflictRow = { check_status: 'waived', waiver_consent_id: 'consent-uuid-001' };
    const r = await checkStageGate(matter(), 'retainer_pending');
    expect(r.allowed).toBe(true);
  });

  it('waived WITH waiver_consent_id allows active', async () => {
    mockState.conflictRow = { check_status: 'waived', waiver_consent_id: 'consent-uuid-001' };
    const r = await checkStageGate(matter(), 'active');
    expect(r.allowed).toBe(true);
  });
