...
export async function loadPublicationPacketsForPeriod(
  ...
  const [{ data: versions }, { data: placements }, { data: artifacts }, standingAuthorization, heldDeliverableIds] = await Promise.all([
    versionIds.length
    ...
    loadUnresolvedClientChangeHoldDeliverableIds(firmId, deliverableIds),
  ]);
