...
function evidence(value: unknown, ancestors = new Set<object>()): GtaProspectQualificationEvidence {
  ...
  if (Array.isArray(value)) return Object.freeze(Array.from(value, (item) => evidence(item, next)));
  ...
  return Object.freeze(Object.fromEntries(Object.entries(value).map(([key, item]) => [key, evidence(item, next)])));
}
function criteria(value: unknown): Readonly<Record<string, GtaProspectQualificationEvidence>> {
  ...
  return evidence(value) as Readonly<Record<string, GtaProspectQualificationEvidence>>;
}
