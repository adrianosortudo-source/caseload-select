...
export function QuestionStage({stage,answers,onEdit,onBack,onNext,onCompare,error,preview}:{stage:StageId;answers:DesiredClientAnswers;onEdit:Change;onBack:()=>void;onNext:()=>void;onCompare:()=>void;error:boolean;preview?:ReactNode}){
 ...
 return <section className="dc-stage" data-ui-component-content={`desired-client-stage-${stage}`}>
  ...
  {stage===3&&<>{multi("dc-goals","What does the client most want to achieve?",entries(GOAL_LABELS),answers.client.goals,v=>onEdit(a=>({...a,client:{...a.client,goals:v as DesiredClientAnswers["client"]["goals"]}})),2,["unknown"])}<details className="dc-optional" open><summary>{route==="established"?"What concern have you heard from these clients?":"What might concern these clients?"}</summary>{multi("dc-concerns",route==="established"?"What concern have you heard from these clients?":"What might concern these clients?",entries(CONCERN_LABELS),answers.client.concerns,v=>onEdit(a=>({...a,client:{...a.client,concerns:v as DesiredClientAnswers["client"]["concerns"]}})),2,["unheard"],[],undefined,false)}{route!=="established"&&<p data-ui-copy="body">We will treat this as something to check.</p>}</details></>}  {stage===4&&<>{multi("dc-reasons","What makes this work worth pursuing?",entries(REASON_LABELS).map(o=>({...o,label:getReasonLabel(o.id as keyof typeof REASON_LABELS,route)})),answers.value.reasons,v=>onEdit(a=>({...a,value:{...a.value,reasons:v as DesiredClientAnswers["value"]["reasons"]}})),3,["undecided"],[],route==="established"?"Think of work you would gladly handle again. Choose up to three reasons. You don't need to describe an individual matter.":"Choose up to three reasons this direction appeals to you. We'll distinguish expectations from experience.")}{radio("dc-fee-effort",route==="established"?"How does the fee compare with the work involved?":"How do you expect the fee to compare with the work involved?",entries({worthwhile:getFeeEffortLabel("worthwhile",route),scoped:getFeeEffortLabel("scoped",route),difficult:getFeeEffortLabel("difficult",route),unknown:getFeeEffortLabel("unknown",route)}),answers.value.fee_effort,v=>onEdit(a=>({...a,value:{...a.value,fee_effort:v as typeof a.value.fee_effort}})))}<details className="dc-optional"><summary>Add commercial detail</summary>{radio("dc-collected-fee",route==="established"?"Typical collected fee, excluding disbursements":"Fee range you are considering, excluding disbursements",entries(COLLECTED_FEE_LABELS),answers.value.collected_fee,v=>onEdit(a=>({...a,value:{...a.value,collected_fee:v as typeof a.value.collected_fee}})),undefined,false)}{radio("dc-team-hours","Typical total team time",entries(TEAM_HOURS_LABELS),answers.value.team_hours,v=>onEdit(a=>({...a,value:{...a.value,team_hours:v as typeof a.value.team_hours}})),undefined,false)}{radio("dc-payment","How predictable is payment?",entries(PAYMENT_LABELS),answers.value.payment,v=>onEdit(a=>({...a,value:{...a.value,payment:v as typeof a.value.payment}})),undefined,false)}</details></>}
  {stage===5&&<>{multi("dc-conditions","What helps your team deliver this work well?",entries(CONDITION_LABELS),answers.delivery.conditions,v=>onEdit(a=>({...a,delivery:{...a.delivery,conditions:v as DesiredClientAnswers["delivery"]["conditions"]}})),3,["unknown"],[],undefined,false)}{radio("dc-capacity","Could the firm take on more of this work now?",entries(CAPACITY_LABELS),answers.delivery.capacity,v=>onEdit(a=>({...a,delivery:{...a.delivery,capacity:v as typeof a.delivery.capacity}})))}<details className="dc-optional"><summary>Set an important limit</summary>{radio("dc-limit","Which condition most often makes this work difficult to support?",entries(LIMIT_LABELS),answers.delivery.limit,v=>onEdit(a=>({...a,delivery:{...a.delivery,limit:v as typeof a.delivery.limit}})),undefined,false)}</details></>}
  {stage===6&&<>{radio("dc-aim","What should this work help the firm become known for?",entries(AIM_LABELS),answers.direction.aim,v=>onEdit(a=>({...a,direction:{...a.direction,aim:v as typeof a.direction.aim}})))}{multi("dc-evidence","What supports this direction?",entries(EVIDENCE_LABELS),answers.direction.evidence,v=>onEdit(a=>({...a,direction:{...a.direction,evidence:v as DesiredClientAnswers["direction"]["evidence"]}})),6,["preference"],[["repeated","few"]])}<details className="dc-optional"><summary>Work to promote less</summary>{radio("dc-less","Which work should the firm promote less?",entries(LESS_LABELS),answers.direction.less,v=>onEdit(a=>({...a,direction:{...a.direction,less:v as typeof a.direction.less,less_note:v==="none"?"":a.direction.less_note}})),undefined,false)}{answers.direction.less&&answers.direction.less!=="none"&&textField("Name the work in a few words",answers.direction.less_note,v=>onEdit(a=>({...a,direction:{...a.direction,less_note:v}})))}</details></>}
