...
export default function StatementStep({ data, onPatch, onContinue, onBack }: Props) {
  ...
  // customClaims is passed for the same reason the brief passes it: a custom
  // claim materialises from that text, and without it every claim the lawyer
  // wrote in their own words would vanish from the anchor list.
  const { survivors } = computeSurvivors(work, data.customClaims);
  ...
  return (
    <div className="card p-6">
      ...
      {survivors.length > 1 && (
        <div className="mb-5">
          <label className="label">{copy.step4.anchorLabel}</label>
