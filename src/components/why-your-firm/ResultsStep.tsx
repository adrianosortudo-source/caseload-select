...
function buildBriefInput(data: WizardData) {
  ...
  return {
    ...
    // Required, not optional: `work` can carry a custom id, and without the
    // text behind it the server would drop a claim the lawyer believed they
    // made. The same object is spread into the report POST body, so the
    // server re-runs the filter on the lawyer's own sentence too.
...
export default function ResultsStep({ data, onBack }: Props) {
  ...
  async function submitGate(e: React.FormEvent) {
    e.preventDefault();
    if (!firstName.trim()) {
      setError(copy.gate.errFirstName);
      return;
    }
    if (!firmName.trim()) {
      setError(copy.gate.errFirmName);
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setError(copy.gate.errEmail);
      return;
    }
    setError(null);
    setSubmitting(true);
    try {
      ...
      if (!json.ok) {
        setError(json.error ?? copy.gate.errSend);
        setDelivered(false);
      } else {
        setDelivered(!!json.emailed);
      }
    } catch {
      setError(copy.gate.errNetwork);
      setDelivered(false);
    } finally {
      setSubmitting(false);
