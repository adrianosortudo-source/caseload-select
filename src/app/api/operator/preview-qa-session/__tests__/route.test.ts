...
describe("POST /api/operator/preview-qa-session", () => {
  it("sets only the short-lived QA cookie and returns no secret-bearing body", async () => {
    const res = await POST(request({ accessSecret: "preview-qa-access-secret-32bytes-0001", bootstrapNonce: "preview-qa-bootstrap-nonce-32bytes-0001" }));
  ...
  it.each([
    ["wrong secret", request({ accessSecret: "wrong", bootstrapNonce: "preview-qa-bootstrap-nonce-32bytes-0001" })],
    ["wrong nonce", request({ accessSecret: "preview-qa-access-secret-32bytes-0001", bootstrapNonce: "wrong" })],
    ["cross-origin request", request({ accessSecret: "preview-qa-access-secret-32bytes-0001", bootstrapNonce: "preview-qa-bootstrap-nonce-32bytes-0001" }, { origin: "https://attacker.test" })],
