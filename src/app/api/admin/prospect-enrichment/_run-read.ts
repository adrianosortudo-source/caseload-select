...
function manifestEntry(row: Record<string, unknown>): ResearchRunEntry {
  ...
  return {
    ...
    errorCodes: entry.errorCodes as string[], packageId: row.package_id === null ? null : storedId(row.package_id), actualPayloadSha256: nullableText(row.actual_payload_sha256, "actual package hash"), packageState: nullableText(row.package_state, "package state"),
...
export async function readRunDetail(input: { runId: string; limit: number; cursor?: string; entryCursor?: string; client?: ReadDatabase }): Promise<ResearchRunDetail> {
  ...
  const [summaryRows, packages, entryRows] = await Promise.all([
  ...
  const inventoryState = missing ? "missing" : row.manifest_state === "finalized" && expectedEntryCount === receivedEntryCount &&
    count(row.manifest_registered_chunk_count) === count(row.manifest_expected_chunk_count) &&
