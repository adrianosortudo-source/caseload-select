...
async function readIdentity(envelopes: readonly ProspectEnrichmentEnvelope[], client: Db): Promise<ComparisonExportInput["identities"]> {
  ...
  for (const [researchKey, versions] of groups) {
    ...
    for (const { subject } of versions) {
      if (subject.databaseFirmId) {
        const rows = databaseRows(await client.from("gta_prospect_firms").select("id,source_record_key").eq("id", subject.databaseFirmId).limit(2));
        if (rows.length !== 1 || subject.sourceRecordKey && rows[0].source_record_key !== subject.sourceRecordKey) contradictory = true;
        else candidates.add(String(rows[0].id).toLowerCase());
      }
      if (subject.sourceRecordKey) {
        const rows = databaseRows(await client.from("gta_prospect_firms").select("id").eq("source_record_key", subject.sourceRecordKey).limit(2));
        if (rows.length !== 1) { if (rows.length > 1) contradictory = true; }
        else candidates.add(String(rows[0].id).toLowerCase());
      }
      if (subject.stableFirmId) {
        const rows = databaseRows(await client.from("gta_prospect_stable_identity_registry").select("firm_id,canonical_domain").eq("stable_firm_id", subject.stableFirmId).limit(2));
    ...
    const firms = databaseRows(await client.from("gta_prospect_firms").select("id,source_record_key").eq("id", firmId).limit(2));
    const registries = databaseRows(await client.from("gta_prospect_stable_identity_registry").select("stable_firm_id,canonical_domain").eq("firm_id", firmId).limit(2));
    if (firms.length !== 1 || registries.length > 1) continue;
    const stableFirmId = registries[0]?.stable_firm_id == null ? null : String(registries[0].stable_firm_id), canonicalDomain = registries[0]?.canonical_domain == null ? null : String(registries[0].canonical_domain);
