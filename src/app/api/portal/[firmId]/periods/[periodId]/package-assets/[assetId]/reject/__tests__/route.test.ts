...
describe("POST package-assets/[assetId]/reject", () => {
  it("rejects a non-operator session", async () => {
    state.isOperator = false;
    const res = await POST(req({ reason: "blurry" }), params());
    expect(res.status).toBe(403);
  });

  it("an Authorization: Bearer header does not bypass a denied operator session", async () => {
    state.isOperator = false;
    const withBearer = { json: () => Promise.resolve({ reason: "blurry" }), headers: { get: () => "Bearer x" } } as never;
  ...
  it("calls rejectCandidate with the reason and returns 200 on success", async () => {
    const res = await POST(req({ reason: "wrong crop" }), params());
    expect(res.status).toBe(200);
    expect(state.lastCall).toEqual([FIRM, PERIOD, ASSET, "wrong crop"]);
  });

  it("422s when the mutations layer returns ok:false", async () => {
    state.result = { ok: false, error: "a rejection reason is required" };
