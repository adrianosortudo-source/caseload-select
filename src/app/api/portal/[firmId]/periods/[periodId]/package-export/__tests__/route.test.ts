...
describe("GET package-export", () => {
  ...
  it("an Authorization: Bearer header does not bypass a denied operator session", async () => {
    ...
    const res = await GET(withBearer, params());
    expect(res.status).toBe(403);
  });
  ...
  it("returns the export bundle shape for an operator", async () => {
    const res = await GET({} as never, params());
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.ok).toBe(true);
    expect(body.humanReadableSummary).toBe("summary");
    expect(body.gatewayManifestJson).toBe("{}");
  });
});
