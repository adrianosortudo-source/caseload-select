...
export async function POST(
  ...
  // getFirmSession (LAWYER ONLY, see the route doc comment above), so this
  // route is already structurally unreachable in support preview. The
  // guard call still runs first so the response carries the uniform
  // support-preview contract (403 + support_preview_read_only) in the one
  // hypothetical where a future change ever widens session admission
  // here, rather than silently depending on an unrelated 401 to keep the
  // boundary honest.
