...
describe("secure continuation", () => {
  ...
  it("returns the caller summary through an exact projection with masked phone", async () => { const result = await (await GET(req())).json(); expect(Object.keys(result).sort()).toEqual(["question", "reviewConfirmed", "revision", "status", "summary"]); expect(JSON.stringify(result)).toContain("Alex"); expect(JSON.stringify(result)).not.toContain("416-555-0142"); expect(Object.keys(result.summary).sort()).toEqual(["answers", "fields"]); });
