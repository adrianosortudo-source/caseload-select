 ...
 *
 * Dalil-style intelligence dashboard for a single lead.
 *
 * Phase 1 (server-rendered, zero new AI calls):
 *   - Score gauge (SVG arc, band colour, priority_index)
 *   - Engagement signal (urgency + stage + days since inquiry)
 *   - Pipeline position (stage track with current position)
 *   - Risk flags (cpi_missing_fields + scoring._flags)
 *   - Key facts (situation_summary + extracted_entities)
 *   - Structured facts sidebar (contact info, value, confidence, timing)
 *
 * Phase 2 (client component, on-demand):
 *   - GPT intelligence brief: watchpoints, demands, next step
 *   - Persisted to scoring._dossier, refreshable
 *
 * Phase 3 (embedded in Phase 2 client component):
 *   - Collapsible intake transcript
 *   - Click watchpoint/demand to highlight source turn
 *
...
// ─── Score gauge (pure SVG, server-rendered) ───────────────────────────────────
...
function ScoreGauge({ score, band }: { score: number | null; band: string | null }) {
  ...
  const arcLen = C * 0.75;                     // 270° arc ~169.6
...
export default async function PortalLeadDetailPage({
  ...
  // ── Fetch intake session (scoring, conversation, entities) ──────────────
  ...
  if (lead.intake_session_id) {
    ...
    if (sessionRow) {
      ...
      if (typeof r === "string" && r.trim()) aiReasoning = r.trim();
      const bxr = scoring._band_x_reason;
      if (typeof bxr === "string" && bxr.trim()) bandXReason = bxr.trim();
      ...
      if (Array.isArray(scoring._flags)) {
        scoringFlags = (scoring._flags as string[]).filter(f => typeof f === "string");
      }
  ...
  // Entity rows for Key Facts panel
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-xs text-black/40">
        <Link href={`/portal/${firmId}/leads`} className="hover:text-black/70">Your Pipeline</Link>
              ...
              {timeToFinalizeSec != null && (
                <span>
                  ...
                  {timeToFinalizeSec < 90
                    ? `${timeToFinalizeSec}s`
                    : `${(timeToFinalizeSec / 60).toFixed(1)} min`}
                </span>
              )}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5">
        ...
        {/* Left column — 5 intelligence panels */}
        <div className="space-y-4">

          {/* Panel 1: Score gauge + AI reasoning */}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="h-1.5 rounded-full bg-black/5 overflow-hidden">
                      <div
                        ...
                        style={{ width: `${((lead.fit_score as number ?? 0) / 30) * 100}%` }}
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <div className="h-1.5 rounded-full bg-black/5 overflow-hidden">
                      <div
                        className="h-full rounded-full bg-navy/70"
                        style={{ width: `${((lead.value_score as number ?? 0) / 65) * 100}%` }}
                      />
                    </div>
          ...
          <div className="bg-white rounded-xl border border-black/5 p-5">
            ...
            {stageIdx >= 0 ? (
                      ...
                      <div key={s.key} className="flex flex-col items-center gap-1 flex-1">
                        <div
                          className={`w-full h-1.5 rounded-full ${
                            i < stageIdx ? "bg-navy" :
                            i === stageIdx ? "bg-navy" : "bg-black/10"
                          }`}
                          style={i === stageIdx ? { background: "#1E2F58" } : {}}
                        />
                      </div>
                    ))}
                  </div>
              ...
              <div className="text-xs text-black/40 italic">
                Stage: {stageLabel} (terminal state)
              </div>
            )}
          </div>
          ...
          {(situationSummary || entityRows.length > 0) && (
            <div className="bg-white rounded-xl border border-black/5 p-5">
              ...
              {situationSummary && (
                <p className="text-sm text-black/70 leading-relaxed mb-3">{situationSummary}</p>
              )}
              {entityRows.length > 0 && (
        ...
        {/* Right sidebar — structured facts */}
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-black/5 p-5">
              ...
              {lead.urgency && (
                <div>
                  ...
                  <dd className="text-xs text-black/75 mt-0.5 capitalize">
                    {(lead.urgency as string).replace(/_/g, " ")}
                  </dd>
                </div>
              )}
              <div>
      ...
      {/* ── Phase 2 + 3: Dossier (client component) ───────────────────── */}
      {lead.intake_session_id && (
        <DossierPanel
          leadId={lead.id}
          firmId={firmId}
          apiPath={`/api/portal/leads/${lead.id}/dossier`}
