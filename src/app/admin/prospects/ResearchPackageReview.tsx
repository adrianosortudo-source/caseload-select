...
export default function ResearchPackageReview({ packageId, initialData }: { packageId: string; initialData?: ResearchPackageReviewView }) {
  ...
  return <div className="min-w-0 space-y-4" data-testid="research-package-review">
    ...
    <header data-ui-component-content="research-package-heading"><h1 className="w-full text-pretty text-xl font-bold text-navy lg:text-2xl" data-ui-copy="heading">Review research package</h1><p className="mt-2 w-full text-sm text-black/60" data-ui-copy="supporting">Tie findings to sources and review profile changes separately.</p></header>
    ...
    {record && <>
      <ResearchPanel title={record.payload.subject.displayName} name="research-package-identity">
        ...
        {record.identityState.includes("conflict") && <ResearchError message="Several firms may use this website. Verify the legal entity before linking." />}
      ...
      <ResearchPanel title="Complete evidence review" name="research-package-items" description={`Review decisions: ${terminal ? Object.keys(savedChoices).length : covered} of ${itemCount} items.`}>
