...
**Build status:** BUILT 2026-06-29 (app commit c30b7c1), hardened 2026-07-02. Schema (`matter_milestone` + `matter_milestone_note` + `quiet_nudge_sent_at` on `client_matters`, plus the `notification_outbox` event_type extension) APPLIED to prod 2026-07-02 (`supabase/migrations/20260702180000_j8_client_matters_milestone_fields_schema.sql`); the milestone-draft route now persists correctly. The pg_cron schedule for `/api/cron/quiet-file-nudge` is still DRAFT ONLY (`supabase/migrations-draft/20260702_quiet_file_nudge_cron_schedule.sql`), deliberately held back until the operator runs the route manually at least once. `POST /api/portal/[firmId]/matters/[matterId]/milestone-draft` (draft composer, Gemini), `MilestoneDraftPanel.tsx` (matter detail, active stage only), `GET /api/cron/quiet-file-nudge` (callable now, not yet on a daily schedule).
