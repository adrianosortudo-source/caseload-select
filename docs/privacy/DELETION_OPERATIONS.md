...
Status: implementation and release gate
...
This section defines the outcome the release must prove. It is not evidence that processor cleanup, backup replay, or the production workflow is already complete.

After a verified request, CaseLoad Select irreversibly removes message content and direct identifiers from active operational systems it controls. The service may retain a minimal audit envelope for system security, delivery-integrity checks, proof that deletion was completed, and non-identifying operational counts.
...
The provisional maximum retention boundary for each audit event is three years from when it occurred. Privacy counsel must approve both this period and the reidentification assessment before the public copy is released.
...
| Encrypted backups | Verify the provider expiry schedule and rehearse reapplication of completed requests before restored data returns to use | CaseLoad Select operator and provider; release gate, not current implementation proof |
...
- A completed request must be replayed after any backup restoration before the restored data becomes operational. This is a release gate until the restore procedure is rehearsed and recorded.
