...
Retention must be enabled with `V2S_RETENTION_ENABLED=true` when the pilot is activated. The existing daily `runDataRetention` job calls service-only `v2s_purge_expired` in bounded batches and reports `voice_screen_inquiries_purged` plus errors. Expiry deletes the entire inquiry and cascading outbox, not just its token. Keep this cleanup gate enabled even after disabling new intake. No purge was executed during implementation.
