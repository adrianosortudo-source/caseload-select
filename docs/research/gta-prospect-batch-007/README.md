# GTA public-roster prospect batch 007

Read-only, first-party public-web roster staging for GTA firms. Observed **2026-09-07**.

## Status and scope

- **25 candidates; 13 exact and 12 at-least roster observations; 0 accepted.** Every row has `workflow_status=candidate` and `accepted=false`.
...
- Each recorded source is a public first-party firm page or public first-party firm PDF. Search was used only to discover sources; each URL in the data is the source reviewed.
- `exact` means the source has a bounded roster that distinguishes lawyers from staff, clerks, students, paralegals, or other non-lawyer roles. `at_least` means the source supports the stated number but lacks a reliable whole-firm roster boundary or needs a fresh roster check. Neither establishes licence status, total firm headcount, legal entity, contactability, advertising activity, or a right to contact a firm.
...
Every candidate was compared by canonical domain and firm name against the PR #231 20-record baseline and batches 001 through 006. No exact canonical-domain or firm-name match was found. The legacy crosswalk remains `unknown_no_stable_crosswalk` for every record; this batch makes no inferred legacy identity, count, or merge.

## Files

- `gta-prospect-batch-007.json` is the complete evidence-aware staging dataset.
- `gta-prospect-batch-007.csv` is a compact review/export view. It does not remove the source, uncertainty, or reconciliation obligations in the JSON.
