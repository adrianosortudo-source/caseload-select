...
supersedes: none (additive to docs/PUBLICATION_READINESS_OPERATING_MODEL.md and DR-105; also additive in spirit to docs/PUBLICATION_OPERATOR_ARCHITECTURE.md, which exists only on the unmerged `feat/publication-operator` branch, not on `origin/main` — see §1 Tier 2)
note: §4.1a and the two new §5 states (surface_adaptation_rule_missing, substantive_adaptation_requires_approval) were added same-day, after DR-105 was locked. §11's verification trail has been updated to describe the current four-file consolidated documentation-only change set, not only this document's original single-file authoring pass.
-->
...
**Agent constraints, restated from DR-105 because this is exactly where an agent could quietly violate them:** never draft, paraphrase, translate, or "lightly edit" a compliance wrapper at publish time, even when the requested change looks obviously safe; never copy the source surface's own wrapper onto a different surface unchanged, since a website-scoped sentence can be false elsewhere; never treat a missing rule as an invitation to write one on the spot; never fold `substantive_adaptation_requires_approval` into a wording tweak an operator can authorize alone — it routes to the same lawyer-approval path as any other substantive content change; never treat the source version's own authorization as this step's concern to relax or re-check on a looser bar than fact A already applies — the registry's `immutable_release_authorized_version` requirement (an individual lawyer approval, or an active standing authorization covering a version not flagged `requires_individual_review`) is exactly fact A's own two-path resolution (§4, fact A), never a separate or looser one; never treat `platform_link_formatting` as license to change a URL, destination, CTA target, or anchor meaning, add or remove a link, or substitute a website URL for a destination surface that requires the native LinkedIn Article URL — see the registry's own scope note for that term.
