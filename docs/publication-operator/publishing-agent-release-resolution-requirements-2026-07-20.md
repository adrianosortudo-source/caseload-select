...
v2 same-day addendum, second pass (2026-07-21): §13.2c corrects the DR-105 mirror introduced in §13.2b, which keyed only on (firm_id, locale) -- safe only by an unverified assumption inherited from elsewhere in the system. It now keys on the registry's real four-part tuple (firm_id, locale, source_surface, destination_surface), deriving the candidate source surface from this deliverable's own content-graph edges rather than assuming one, and fails closed (source_path_unverified) when that edge cannot be resolved. See §13.2c.
v2 same-day addendum, third pass (2026-07-21): §13.2d corrects §13.2c's source-edge derivation, which checked only that a firm_website placement existed -- placements are not version-scoped in this schema, so an old placement could satisfy that check after the content had changed. The source edge must now also be individually release-authorized (approved_version_id === current_version_id) AND backed by a webpage publication_artifacts row bound to that exact version -- never merely a placement's existence. See §13.2d.
v2 same-day addendum, fourth pass (2026-07-21): §13.2e corrects §13.2d's release-authorization check, which required individual approval only -- conflicting with this system's already-established two-path model (individual approval OR an active standing publishing authorization, unless requires_individual_review overrides it). The source edge now respects both legitimate paths via a new shared pure helper, `isVersionReleaseAuthorized()`, faithfully ported from the canonical RPC's logic. See §13.2e.
v2 same-day addendum, fifth pass (2026-07-21): §13.2f corrects the scope of §13.2e's fix, which unified fact 7's OWN authorization check but left two other consumers of "is this version release-authorized" -- fact 1 and the reused `buildPreflightReport`/`existingPreflightGate` -- still answering that question with the old individual-approval-only logic. A standing-authorized release could pass fact 7 while fact 1 and the existing gate both still called it unauthorized: three different verdicts about the same fact within one audit. `isVersionReleaseAuthorized()` moved to its own dependency-free module (`release-authorization.ts`) so every consumer, including `buildPreflightReport` (extended with a new, backward-compatible optional input), reads the exact same result. See §13.2f.
-->
...
| `compliance_wrapper_missing` | A `linkedin_article` placement resolves this against the registry's own **exact four-part tuple** — `firm_id + locale + source_surface + destination_surface`, matched with strict equality, no partial match on any subset of the four — where the source surface itself must be resolved from a `firm_website` placement, an `"article"` role, release-authorization of the exact current version via either legitimate path (individual lawyer approval, or an active standing publishing authorization unless the version is flagged `requires_individual_review`, which overrides it unconditionally), AND a `webpage` artifact bound to that exact version id (never merely a placement's existence, which is not version-scoped in this schema). Reports one of three outcomes, distinguished by evidence, never collapsed into one message: (a) **source surface unresolved/unsupported** — any one of the four source-edge checks fails (`source_path_unverified`, `needs_human_confirmation`; fails closed before the tuple is ever looked up); (b) **wrapper absent** — the tuple resolves, but no DR-105 rule is documented for that exact tuple (`blocks_today`, a real doctrine gap needing operator+lawyer authorship); (c) **documented but not runtime-bound** — a rule IS documented for the exact tuple (named by `rule_id`), but no code path applies or binds it to a release (`system_improvement`, engineering-only). An `email_delivery` placement resolves it from configured branding, or branding present but no canonical registered legal-wrapper text. | Same underlying gap as §7's `surface_adaptation_rule_missing`, extended to also cover email's own compliance wrapper (which §7 did not address) and, as of 2026-07-21, corrected five times same-day: stop conflating "no reader exists" with "no rule exists"; stop matching on firm+locale alone; stop treating an unversioned placement's mere existence as a resolved source edge; stop treating individual approval as the only legitimate release-authorization path; stop letting fact 1 and `existingPreflightGate` answer that same authorization question with a different, uncoordinated local check — see §13.2b, §13.2c, §13.2d, §13.2e, §13.2f. |
...
#### 13.2c Policy correction, 2026-07-21 (same day, second pass): the DR-105 mirror must match the registry's own four-part key, not a two-part shortcut

A second independent review, the same day, found that §13.2b's fix was still incomplete. The mirror introduced in §13.2b keyed only on `(firmId, locale)` — safe only by *assumption*, because in this codebase's current content-graph rule a `linkedin_article` placement can only ever republish a `website_article`. That assumption was never verified by the audit itself; it was inherited, silently, from a different part of the system. The registry's own rule is keyed on all four fields (`firm_id`, `locale`, `source_surface`, `destination_surface`; confirmed directly against `surface-presentation-adaptation-registry.md`'s own rule block) and this audit's mirror now matches that exactly:

- The mirror's rule shape gained two required fields, `sourceSurface: "website_article"` and `destinationSurface: "linkedin_native_article"` (the registry's own vocabulary — distinct from, and not spelled the same as, `content_placements.destination`'s `"linkedin_article"` enum value; `registryDestinationSurfaceFor()` is the one narrow, explicit bridge between the two vocabularies).
- The lookup, `findKnownDr105Rule()`, requires exact equality on all four fields. There is no wildcard, no fallback, no "same firm/locale is close enough" branch, and it is exported specifically so this exact-match discipline is independently, directly testable (§13.6/tests) rather than only reachable through the full placement-routing pipeline.
- **The candidate source surface is now derived, never assumed.** Before any tuple lookup runs, the audit checks this deliverable's *own* `content_placements` rows for a `firm_website` destination (the actual content-graph edge the content-graph rule requires) and confirms the deliverable's role is `"article"` (a `firm_website` placement on a `"landing_page"`-role deliverable is a different, currently-unsupported source surface, not `website_article`). If that edge cannot be resolved — no `firm_website` sibling placement at all, or one exists but the role is wrong — the audit fails closed to `source_path_unverified` (`needs_human_confirmation`) and never proceeds to the rule lookup at all. This is the one behavioral difference from §13.2b: a linkedin_article placement can now resolve to three outcomes, not two (source unresolved/unsupported; wrapper absent for the resolved tuple; wrapper documented for the resolved tuple but not runtime-bound).

The audit itself now verifies its own authorization boundary end to end — firm, locale, and both surfaces — rather than trusting an invariant enforced elsewhere in the system to hold silently. Still no schema change, no new table, no runtime registry reader: `findKnownDr105Rule()` remains the same kind of temporary, hand-maintained, exact-match policy projection §13.2b already described, now simply matching the registry's real key in full rather than a subset of it. Its replacement is still, and only, the controlled runtime registry reader preflight design §10 item 4 specifies — never a wider or smarter hand-maintained table.

#### 13.2d Adversarial check, 2026-07-21 (same day, third pass): the source edge must be bound to the SAME version being republished, not merely exist

§13.2c's "derive the source surface from this deliverable's own `content_placements`" fix checked for a `firm_website` placement's *existence*, but `content_placements` rows are not version-scoped in this schema — a placement records intent for a destination, not which version's content it currently represents. An old website placement, created for a version that has since been superseded by a new approved edit, would have satisfied §13.2c's check exactly as well as a genuinely current one. `resolveWebsiteArticleSourceSurface()` now resolves the source edge from stronger, version-bound evidence, checked in order, any one of which fails closed:

1. A `firm_website` placement exists on this deliverable (`no_website_placement` otherwise).
2. The deliverable's role is `"article"` (`wrong_role` otherwise).
3. The version being evaluated is individually release-authorized — `approved_version_id === current_version_id` (`version_not_authorized` otherwise). This re-checks the same fact fact 1 already gates on, deliberately: fact 7 must never treat an unauthorized version as a valid source regardless of what any placement or artifact record says, and relying on fact 1 to have already caught it would be exactly the kind of silent cross-fact assumption this whole correction series exists to close. Only the individual-approval path is checked here; an active standing publishing authorization (the two-path bar fact 1 itself applies) is a separate, firm-level fact this narrow check has no input for, so a standing-authorization-eligible version that has not also been individually approved is conservatively treated as *not* eligible for this specific check — fail-closed, never the permissive direction.
4. A `webpage`-type `publication_artifacts` row is bound to that *exact* version id (`no_version_bound_artifact` otherwise) — the actual evidence that a website rendering of the current content exists, not merely that a placement (intent) for the destination exists. An artifact registered for an older version is stale evidence and must never be read as this release's source edge, even though the placement itself is unchanged.

Only step 4's outcome fails to already have a name from earlier work — `no_website_placement` and `wrong_role` are unchanged from §13.2c. All four resolve to the same classification and impact (`source_path_unverified`, `needs_human_confirmation`), with the finding's evidence naming the specific step that failed, matching this addendum's own §3 discipline of always distinguishing *why* something is unresolved rather than reporting one blended "unresolved" message.

#### 13.2e Policy correction, 2026-07-21 (same day, fourth pass): the version-authorization check must respect both legitimate release-authorization paths, not individual approval alone

A fourth review, the same day, found that §13.2d's step 3 — `approved_version_id === current_version_id` — checked only the individual-approval path. That conflicts with a release-authorization model this system had already established elsewhere: a version is release-authorized when EITHER (a) the firm's lawyer has individually approved it, OR (b) the firm holds an active standing publishing authorization AND the version is not flagged `requires_individual_review` (an operator-set override that always takes precedence over standing authorization, checked before standing authorization is ever consulted). This exact two-path bar is what `claim_placement_for_publish()` — the RPC backing the real publish action — already implements (`supabase/migrations/20260717230956_standing_publishing_authorization.sql`); §13.2d's step 3 silently narrowed it to path (a) only, which would have caused this audit to report `source_path_unverified` for legitimately publishable content on any firm actually using standing authorization.

No existing, importable, read-only function implements this two-path bar — the only real implementation lives inside `claim_placement_for_publish()`, which performs a database write (it creates a `publication_placement_claims` row) and must never be called from a read-only audit. The fix is a new, pure, exported helper, `isVersionReleaseAuthorized()`, that faithfully ports the RPC's branching logic (never its write) with the migration file and line range cited directly in its own doc comment as the authoritative source of truth. `resolveWebsiteArticleSourceSurface()`'s step 3 now calls this helper instead of the narrower equality check; steps 1, 2, and 4 are unchanged from §13.2d. The loader threads a new `standingAuthorizationActive` fact — this firm's current standing-authorization state, read once per firm via the existing `getStandingAuthorizationState()` — into the audit input for step 3 to consult; nothing else in this stage reads or writes standing-authorization data.

The `version_not_release_authorized` finding's evidence text is no longer one blended "not individually approved" message. It now names the specific reason, distinguished from the raw inputs: no current version at all; a version flagged `requires_individual_review=true` overriding an active standing authorization; or a version for which neither path is satisfied (not individually approved, and either no active standing authorization or none applicable).

This correction is scoped narrowly to fact 7's source-edge check, per explicit instruction. Two other places in this codebase still use individual-approval-only logic and are knowingly NOT touched by this pass: `buildPreflightReport`'s own `existingPreflightGate` (reused verbatim on every audit, per §13's design), and fact 1's own `resolveFact1And2SourceAndSurface` (the `source_path_unverified` check the "approved_version_id drifted from current_version_id" test exercises). Both are pre-existing simplifications elsewhere in this system, not introduced or corrected by this addendum, and remain out of scope until a future pass addresses them explicitly.

#### 13.2f Policy correction, 2026-07-21 (same day, fifth pass): every release-graph authorization consumer must read ONE canonical result, not one-per-consumer local interpretations

§13.2e's own closing paragraph named the gap it was leaving open, and a fifth review the same day required closing it before independent review: fixing fact 7 alone was insufficient, because fact 7 was never the only place inside this audit that answers "is this version release-authorized." Three separate consumers each answered that question independently:

- Fact 7 (`resolveWebsiteArticleSourceSurface`) — fixed in §13.2e to the two-path bar.
- Fact 1 (`resolveFact1And2SourceAndSurface`) — still compared `approved_version_id !== current_version_id` directly, ignoring standing authorization entirely.
- `existingPreflightGate` (this audit's carried-forward copy of `buildPreflightReport`'s `mayPublish`/`reason`) — `buildPreflightReport` itself still gated on `deliverable.status !== "approved"` and the same raw ID inequality.

The practical consequence: a release legitimately authorized only through an active standing publishing authorization could resolve fact 7 as authorized while fact 1 and `existingPreflightGate` both, in the same audit output, reported it as unauthorized -- three contradictory answers to one fact about one release. The same release cannot have different authorization meanings in different parts of one preflight; a self-contradictory audit is not more conservative than a wrong one, it is simply not trustworthy.

**The fix.** `isVersionReleaseAuthorized()` moved out of `release-graph-audit.ts` into its own new, dependency-free, no-I/O module, `src/lib/release-authorization.ts` (mirrors this codebase's existing pure/I-O module-split convention). This was necessary, not cosmetic: `release-graph-audit.ts` already imports `buildPreflightReport` from `publication-preflight.ts` (composes with it, per this document's own architecture), so `publication-preflight.ts` could not import the helper back from `release-graph-audit.ts` without an illegal circular dependency. The relocated module is the single, canonical authorization interpreter for this codebase; `release-graph-audit.ts` re-exports the same symbols for backward compatibility with existing importers.

The result type is no longer a bare boolean. `ReleaseAuthorizationResult` is a discriminated union over six named outcomes -- `individually_approved`, `standing_authorization`, `blocked_requires_individual_review`, `standing_authorization_inactive`, `approved_version_mismatch`, and `no_release_authorization` (this last one reserved, not currently reachable by any input this codebase's data supplies today -- see the module's own doc comment for why: `getStandingAuthorizationState()` exposes only a current boolean, never whether standing authorization was ever configured at all versus configured-then-revoked, so this codebase cannot yet distinguish those two causes; named now so a future caller that gains that finer signal has somewhere to put it, exactly this document's own established precedent for a defined-but-dormant state, per §13.2's `compliance_wrapper_missing` "bound" outcome) -- carrying `authorized`, `authorizationPath`, a human-readable `reason` that always names the actual path (never a bare "approved"; a standing-authorized version is never described as individually approved, and standing authorization is never described as approval of legal content), and the raw evidence (`approvedVersionId`, `targetVersionId`, `versionRequiresIndividualReview`, `standingAuthorizationActive`) the decision was made from.

`resolveAndAuditReleaseGraph` now computes this result exactly ONCE per audit (when a current version exists) and passes the same object to all three consumers:

1. **Fact 1** (`resolveFact1And2SourceAndSurface`) now reports `source_path_unverified` exactly when `!releaseAuthorization.authorized`, with `rootCause` naming the canonical `kind` and `factualEvidence` set to the canonical `reason` string, verbatim -- never re-derived.
2. **Fact 7** (`resolveWebsiteArticleSourceSurface`) takes the same object as a parameter (replacing the narrower `standingAuthorizationActive: boolean` parameter §13.2e added) instead of calling the helper itself.
3. **`buildPreflightReport`** gained a new, OPTIONAL, backward-compatible input, `releaseAuthorizationByDeliverableId?: Record<string, ReleaseAuthorizationResult>`. When a caller supplies an entry for a deliverable (today, only `release-graph-audit.ts` does), `reportOnePlacement` uses `releaseAuthorization.authorized`/`.reason` in place of its own `status !== "approved"` / ID-equality checks. When omitted -- every other existing caller of `buildPreflightReport` (the live `/publication-preflight` and `/claim` API routes, via `publication-preflight-loader.ts` and `publication-placement-claims.ts`) -- this function falls back to its original individual-approval-only check, UNCHANGED, byte-for-byte, including the exact reason strings `publication-preflight.test.ts` already locks in.

**Why the optional-parameter boundary, not a full rewrite of `buildPreflightReport`.** `buildPreflightReport` is not release-graph-audit's own code; it is pre-existing, shared infrastructure this stage explicitly composes with rather than re-derives (§13's own architecture section), and it backs the live claim/publish flow for every firm, not only this audit. Widening its default behavior for every caller -- including the two live API routes -- to the two-path model is a legitimate, separate decision with its own blast radius (it would require those routes' own loaders to also fetch and thread standing-authorization state), explicitly outside "continue only the Publishing Agent release-graph audit workstream." The additive, opt-in design lets the release-graph audit's own three consumers agree completely -- the actual contradiction this pass exists to close -- without silently changing production gating behavior for callers this workstream was not asked to touch. This boundary is a deliberate scope decision flagged here for independent review, not an oversight; note that `claim_placement_for_publish()` (the RPC that is the REAL authority behind any live publish, per `publication-preflight.ts`'s own docstring) already correctly implements the two-path bar today, so this gap was in the ADVISORY preflight report only, never in actual publish enforcement.

**What did not change.** Release authorization remains one fact among ten, deliberately independent of the others: an authorized version can still fail on a missing or stale exact-version webpage artifact (`no_version_bound_artifact`/`source_artifact_version_mismatch`, fact 7's step 4, unchanged from §13.2d), a missing placement, an absent compliance-wrapper rule, missing channel credentials, or an unresolved destination -- authorization answers only "is this specific version allowed to be released," never "is this release ready in every other respect." No new authorization path was invented; the two-path bar and the `requires_individual_review` override are exactly the ones §13.2e already described and the migration already implements -- this pass only removed the duplicated, conflicting LOCAL interpretations of that same, unchanged rule.

**Standing-authorization state quality (verified, not changed).** `getStandingAuthorizationState(firmId)` (`standing-publishing-authorization.ts`) reads `standing_publishing_authorizations` -- an append-only event log (`event_seq bigint generated always as identity`) -- filtered by `firm_id`, ordered `event_seq desc`, `limit(1)`: the single latest event for that firm, never a broader scan. The `event` column is `CHECK`-constrained to exactly `'enabled'` or `'disabled'`; there is no separate expiry mechanism in this schema (no `expires_at` field), so "revoked" is represented simply as a later `'disabled'` event appended to the same log -- "latest wins" is the entire model. `active` is `true` only when the latest event's `event = 'enabled'`; a firm that has never touched this feature at all (`data` is `null`) also resolves to `active: false` via the loader's `?? false` -- both "never configured" and "explicitly revoked" fail closed identically, which is why `no_release_authorization` (a hypothetical distinction between those two causes) is not currently reachable. Firm identity is enforced at the query (`.eq("firm_id", firmId)`) and reinforced by this table's own unique index on `(firm_id, event_seq desc)`. The release-graph loader (`release-graph-audit-loader.ts`) calls this function exactly ONCE per period-load, before the per-deliverable loop, and reuses the same boolean for every deliverable and placement audited in that call -- never re-fetched or re-derived per-placement. Like `buildPreflightReport`'s own `mayPublish`, this audit's authorization result is a read-only snapshot, advisory rather than authoritative -- `claim_placement_for_publish()` independently re-reads standing-authorization state under lock at actual claim time, exactly as it already does for every other precondition this audit reports on.
