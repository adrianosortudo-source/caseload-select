...
| `compliance_wrapper_missing` | A `linkedin_article` placement resolves this against the registry's own **exact four-part tuple** — `firm_id + locale + source_surface + destination_surface`, matched with strict equality, no partial match on any subset of the four — and reports one of three outcomes, distinguished by evidence, never collapsed into one message: (a) **source surface unresolved/unsupported** — this deliverable's content graph does not resolve a `website_article` source edge for this placement at all (`source_path_unverified`, `needs_human_confirmation`; fails closed before the tuple is ever looked up); (b) **wrapper absent** — the tuple resolves, but no DR-105 rule is documented for that exact tuple (`blocks_today`, a real doctrine gap needing operator+lawyer authorship); (c) **documented but not runtime-bound** — a rule IS documented for the exact tuple (named by `rule_id`), but no code path applies or binds it to a release (`system_improvement`, engineering-only). An `email_delivery` placement resolves it from configured branding, or branding present but no canonical registered legal-wrapper text. | Same underlying gap as §7's `surface_adaptation_rule_missing`, extended to also cover email's own compliance wrapper (which §7 did not address) and, as of 2026-07-21, corrected twice same-day to stop conflating "no reader exists" with "no rule exists," then to stop matching on firm+locale alone — see §13.2b and §13.2c. |
