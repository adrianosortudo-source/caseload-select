...
| The relocation clause | `legacy_unreconciled` | 13 active (1 excluded/archived) | 3 (1 webpage, 1 PDF, 1 landing page webpage route: partial, see note below) | 4 (all confirmed 404 deploy gaps or never-authored PT content) | 5 (3 GBP, 2 LinkedIn) | 1 (PT companion article) | Dry-run assessment complete; artifact registration not started |
...
Totals across the 3 in-scope periods: 21 active deliverables, 11 confirmed `verified_and_bindable` or partially so (10 fully `verified_and_bindable`, webpage/PDF evidence live-checked 2026-07-15, plus 1 `verified_and_bindable_partial`, the relocation-clause landing page's webpage route: confirmed live, but that same deliverable's delivery-email and thank-you-page requirements remain `inaccessible_with_current_permissions`, not bindable), 4 confirmed `missing` (all traced to a specific, named, pre-existing cause, either a `drg-law-website` deploy gap or content that was never authored in any language), 5 `inaccessible_with_current_permissions` (LinkedIn/GBP, this session has no account access to either platform), 1 `pending_legal_approval` (a specific deliverable, `b767ef14`, genuinely awaiting Damaris's review, untouched by any migration in this branch).
...
1. **Artifact registration (all 3 in-scope periods, 11 deliverables: 10 fully `verified_and_bindable`, 1 `verified_and_bindable_partial`).** An operator must personally load each verified-live URL or download each verified-live PDF and register it via the manual insert path, then run the deliverable's reconcile-artifacts action. The one partial item (the relocation-clause checklist landing page) has a confirmed live webpage route, but its delivery-email and thank-you-page requirements remain `inaccessible_with_current_permissions`, so registering it covers only the route, not the full requirement set. This session verified the evidence exists; it did not, and should not, register it on the operator's behalf, since artifact registration is itself an attestation that a human checked the live result.
2. **`drg-law-website` deploy gap (3 confirmed 404s, all in the relocation-clause period).** `demolition-clause-ontario` (EN article), the relocation-clause checklist PT PDF, and the relocation-clause checklist PT landing page all have complete, authored source in the `drg-law-website` repository that has not been deployed (that project ships via manual `vercel --prod` deploys, no git integration). This is a pre-existing production gap independent of Publication Readiness; flagged as its own operator follow-up. (A 4th `missing` item in the same period, the PT counsel-note, is a different class of gap: no PT source was ever authored for it anywhere, so there is no page to 404 on; see item 5(a) below.)
