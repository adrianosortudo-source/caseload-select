...
| `pages_show_list` | Include as a Meta-required technical dependency | No `/me/accounts` or Page-enumeration product flow exists under `src/`. | Meta's current Permissions Reference lists this as a dependency of both legacy `instagram_manage_messages` and `pages_messaging`. It is requested only to support those messaging capabilities, not broad Page enumeration. |
