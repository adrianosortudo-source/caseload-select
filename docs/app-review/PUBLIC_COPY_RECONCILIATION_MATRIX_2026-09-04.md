...
**Status:** Review matrix only. No public copy was changed. Counsel approval and the final technical closeout remain open.
...
HTTP availability does not prove that the wording is approved or legally supportable. The current production source basis is merge commit `6f6c59330d94d84b1fc3bc63fb76d8830d3c8644`.
...
| `/data-deletion` says restored data cannot return to use until deletion replay is complete. | The service-only circuit, external encrypted registry, backfill, controlled replay, and CI restore simulation support the fail-closed design. Production remains locked pending the current-registry audit and final activation/open postflight. | Treat this as provisional until technical closeout finishes; then capture the final production evidence. |
...
1. Finish the current-registry audit, final fictional end-to-end verification, and production activation/open postflight.
2. Obtain the dated counsel decision requested in `../privacy/PRIVACY_COUNSEL_APPROVAL_REQUEST.md`.
3. Turn each matrix row into an approved keep, narrow, replace, or remove decision.
4. Implement source changes through a pushed PR and Git-integrated deployment; do not edit production directly.
5. Verify `/privacy`, `/terms`, and `/data-deletion` signed out and preserve the rendered text or screenshots.
6. Update the final Meta draft only after the published wording matches the evidence.
