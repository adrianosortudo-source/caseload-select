...
| Ses.21 Publication Operator (dry-run only) | Task-directed build of the "Publication Operator": a constrained, deterministic, DRY-RUN-ONLY release executor sitting on top of the existing content_placements/publication_placement_claims/publication_receipts backend (built one session before this one, never previously exposed through any UI or unified manifest). Full architecture: `docs/PUBLICATION_OPERATOR_ARCHITECTURE.md`. Operator runbook: `docs/runbooks/publication-operator-runbook.md`. Real-data dry-run pilot report: `docs/reconciliation/publication-operator-dry-run-founder-vesting-2026-07-18.md`. **Zero schema/migration changes.** **WS1** (`lib/publication-execution-manifest.ts` + `-loader.ts`): one immutable `PublicationExecutionManifest` per (approved version, placement) pair, server-derived only -- deterministic sha256 idempotency key, hashed body, ordered/hashed asset list, a prospective (never-authoritative) read-only re-derivation of `claim_placement_for_publish()`'s individual-approval/standing-authorization gate, a destination base URL resolved only from PRIOR VERIFIED evidence (never guessed, never a hardcoded per-firm domain), surfaces an existing active claim on the placement so a dry run never implies a duplicate-publish is safe. **WS2** (`publication-preflight-status.ts` + `publication-destination-validators.ts`): a 7-way status taxonomy (ready / blocked_content / blocked_missing_configuration / blocked_authorization / blocked_destination_validation / already_published / ambiguous_external_state) layered on top of the existing binary mayPublish/reason preflight report, plus real pre-publish format checks (LinkedIn feed post 3,000 chars, LinkedIn article 220-char headline / ~110k-char body, GBP 1,500-char body + required image, website slug shape) sourced from current public platform documentation, distinct from the existing post-publish `channel-validation.ts` evidence check. **WS3** (`publication-adapter.ts`): Website/LinkedIn/GBP/Email adapters (`validateConfiguration`/`preflight`/`renderDryRun`/`execute`/`reconcile`/`normalizeReceipt`); `execute` always returns `ok:false` and never calls `fetch`, proven by a fetch-spy test across every destination including with a forged live-execution env flag (no live-call implementation exists to gate -- none of LinkedIn, GBP, or a website-deploy integration exist anywhere in this codebase, confirmed at discovery); `reconcile` delegates straight to the existing SSRF-safe `channel-validation.ts`, never duplicated. **WS4**: one read-only route, `GET .../deliverables/[deliverableId]/placements/[placementId]/publication-execution-manifest` (operator-only, firm-scoped, 404s on a cross-firm placement id), assembling manifest + preflight status + configuration + dry-run render; never calls the real claim RPC or writes a receipt. **WS5**: operator Publication Queue at `/admin/content-studio/publication-queue` (firm/period picker, status-count summary, placements table via the existing batched preflight report) drilling into a per-placement detail page (approved-version preview, destination/authorization, asset/hash table, redacted dry-run action, claim/receipt history, raw manifest) -- verified live in a browser against real production data (DRG Law), not fixtures. **WS6** (security pass): reused `ssrf-fetch.ts`/`channel-validation.ts` rather than any new fetch code (zero raw `fetch()` calls anywhere in this release's own files); found and fixed one real defense-in-depth gap live during review -- the detail page fetched another firm's claim/receipt rows in parallel with the (firm-scoped) manifest load, discarded-but-fetched before the 404 check ran; resequenced strictly after the firm-scope check. **WS7**: 117 new targeted tests (manifest/preflight/adapters/route) plus the full existing suite, 5652/5652 passing, zero regressions, `tsc --noEmit` clean; local `next build` (both Turbopack and webpack) fails in this specific nested-git-worktree location with filesystem-level `readlink`/workspace-root errors, proven to be a pre-existing, code-independent environment limitation by reproducing the identical failure class against an untouched sibling worktree on an unrelated file -- matches this repo's own documented Turbopack-on-D:-drive precedent ("Vercel's prod build uses its own toolchain and is unaffected"). **WS8** (real-data pilot): `content_placements` has zero rows in production, for every firm, not just DRG (confirmed via read-only Supabase query) -- the newer placement-based backend from the prior session has never actually been populated for any real firm's content. All 13 real Founder Vesting deliverables carry complete legacy `deliverable_role`/`publication_destination`/`publication_path` metadata but no `content_placements` row and are all currently `status='in_review'` with no approved version (a live snapshot distinct from the `PUBLICATION_READINESS_OPERATING_MODEL.md` doctrine doc's four-day-older evidence table). The pilot reports this honestly as the real finding rather than fabricating placements to make the demo look complete -- creating real placement rows would be a production write, prohibited outright by this release's hard safety boundaries, not something to ask permission to work around. Engine correctness is instead proven via the 117 targeted tests against realistic fixture data plus live UI verification of the true empty/gap state. Zero external writes, zero fabricated receipts, zero items marked published, confirmed. | DONE |
