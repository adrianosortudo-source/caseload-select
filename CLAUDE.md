...
### CaseLoad Select's own prospects (DR-114)

`caseload_prospects` holds CaseLoad Select's own inbound demand from the public "Start a conversation" flow (name, firm name, email, province), so it needs the same posture as a client firm's leads. It gets it the same way: anonymised, never deleted.

- **Deletion is structurally unavailable, and now says so.** The table shipped with `ON DELETE CASCADE` on `caseload_prospect_consent_log.prospect_id` alongside an unconditional append-only trigger on that same child, which contradict each other: the cascade's DELETE hits the guard and the whole statement fails with `P0001 this table is append-only`. The FK is now `ON DELETE RESTRICT`, and `caseload_prospects` carries `trg_block_caseload_prospect_delete` whose error names the supported path instead of raising a foreign-key violation.
- **Consent evidence is retained intact, `ip_address` and `user_agent` included.** CASL section 13 puts the burden of proving consent on the sender, so the record that a past contact was lawful must survive an erasure request rather than be destroyed by it. Once the parent row is anonymised the consent row carries no name, email, firm name or province, so those two fields point at a subject the data can no longer identify. Do not add a deletion exception to `block_append_only_mutation()` for this table; an append-only table with a deletion function attached is only as append-only as that function's callers.
- **One definition of the replacement.** `public.anonymize_caseload_prospects(p_prospect_id, p_email, p_before, p_reason)`, SECURITY DEFINER, service_role EXECUTE only, exactly one selector per call, idempotent. It replaces `name` / `firm_name` / `email` / `province` with `[anonymized]`, nulls `practice_area_other` and `prompt_reason_other` (free text the visitor typed, which can name people), keeps the closed-option answers and `outcome` so funnel counts stay correct, and stamps `anonymized_at` + `anonymization_reason` (`subject_request` | `retention_sweep` | `internal_test_record`).
- **Two callers.** `POST /api/admin/caseload-prospects/purge` (operator-only: bearer `CRON_SECRET` / `PG_CRON_TOKEN` or an operator session; body takes exactly one of `prospect_id`, `email`, `before`) and `runProspectRetentionSweep` inside the daily `runDataRetention`, at `PROSPECT_RETENTION_DAYS` (730) from `submitted_at`. Unlike the leads purge, this route reports `anonymized_count`: the caller is already the operator, so a zero is a useful signal they typed the wrong address rather than something to hide behind an enumeration defence.

Key files: `src/lib/caseload-prospect-erasure.ts`, `src/app/api/admin/caseload-prospects/purge/route.ts`, migration `20260808120000_caseload_prospect_erasure.sql`, `src/lib/__tests__/caseload-prospect-erasure.integration.test.ts` (real Postgres, gated on `DIRECT_DATABASE_URL`).
...
## Firm Asset Ownership Register (DR-111, 2026-08-06)

Operator console at `/admin/firms/[firmId]/ownership`, answering one question for a client firm: does the firm currently control the marketing assets its growth depends on. Distinct from `04_Playbooks/05_Operations/ACTS_Day1_OwnershipMatrix_v2.md`, which records the GO-FORWARD decision (which of three modes CaseLoad Select and the firm use for a surface during the engagement). This register is the BEFORE snapshot, re-run at onboarding and again at offboarding.

- **Catalogue in code, not a table column per asset.** `src/lib/asset-ownership-catalogue.ts` lists ~25 assets across six categories (brand and content, domain and website, search and local presence, advertising and attribution, CRM/intake/relationships, social and communications). Adding an asset later is a code edit, no migration.
- **Four current-state statuses**, not a score: `firm_controlled`, `shared_access`, `provider_controlled`, `unknown`. `src/lib/asset-ownership-pure.ts` derives the prioritized transfer/repair list (provider controlled first, then unknown, then shared access; firm controlled and already-done rows drop off).
- **No credential column exists on the table by design.** The PATCH route explicitly rejects any body key that looks like a password, token, or API key, rather than silently dropping it, so a caller can never believe a credential was recorded when it was not.
- **Two review phases** (`onboarding`, `offboarding`) on the same table, `UNIQUE (firm_id, review_phase, asset_key)`; re-seeding is idempotent, never resets an existing row.
- Operator-only in this phase (DR-111): no firm-facing intake form ships yet. Antecedent review with the full reusable-row field model and the open decisions: `00_System/08_Drafts/CaseLoadSelect_OnboardingForms_OwnershipInventory_Review_v1.md` sections 4 through 8.
- Key files: `src/lib/asset-ownership-catalogue.ts`, `asset-ownership-pure.ts`, `asset-ownership.ts` (IO, guarded reads); `src/app/api/admin/firms/[firmId]/ownership/route.ts` (GET register + transfer list, POST seed) and `.../[rowId]/route.ts` (PATCH one row); `src/app/admin/firms/[firmId]/ownership/page.tsx` + `src/components/admin/OwnershipRegisterPanel.tsx`.
...
31. `20260806140000_firm_asset_ownership.sql`: Firm asset ownership register (DR-111). Creates `firm_asset_ownership` (RLS enabled + forced, anon/authenticated/PUBLIC revoked, `set_firm_asset_ownership_updated_at` trigger with `search_path` pinned). **NOT YET APPLIED to prod.** Per the migration-lineage process rule (DR-109; `docs/audits/MIGRATION_LINEAGE_CLOSURE_2026-07-27.md` section 5), no MCP `apply_migration` and no direct-from-local-checkout `db push` -- the only sanctioned path is commit, push, PR, merge, then a deliberate operator-run `supabase db push`. All reads through `src/lib/asset-ownership.ts` are guarded (empty register, not an error, when the table is absent), so the admin page at `/admin/firms/[firmId]/ownership` is safe to deploy ahead of the migration landing.
32. `20260807120000_caseload_prospects.sql`: CaseLoad Select's own prospect capture from the public "Start a conversation" flow. Creates `caseload_prospects` (typed column per answer, not a JSON blob) and the append-only `caseload_prospect_consent_log`, plus the atomic `insert_caseload_prospect_with_consent` RPC. Both RLS enabled + forced, anon/authenticated/PUBLIC revoked. APPLIED to prod 2026-08-08.
33. `20260808120000_caseload_prospect_erasure.sql`: PIPEDA erasure path for `caseload_prospects` (DR-114). Swaps the child FK from `ON DELETE CASCADE` (a dead letter: the cascade's DELETE always hit the child's append-only guard) to `ON DELETE RESTRICT`, adds `anonymized_at` + `anonymization_reason` with a paired CHECK, adds `block_caseload_prospect_delete()` whose error names the supported path, and adds the `anonymize_caseload_prospects()` SECURITY DEFINER function (service_role only). **NOT YET APPLIED to prod.** Same DR-109 path as item 31: commit, push, PR, merge, then a deliberate operator-run `supabase db push`. The reading code degrades safely ahead of it (the purge route returns a 500 with the missing-function message rather than reporting a false erasure, and `runDataRetention` records the failure in `errors` without failing the rest of the sweep), but the erasure path itself does not exist in prod until the migration lands.
