...
compile also creates expected-run-manifest.json, expected-run-manifest-chunks.jsonl and held-candidate-evidence.jsonl. Every candidate-specific entry with no package preserves its complete original candidate object plus structured issue code, source path and reason in the held-evidence artifact. Its SHA-256 commitment is included in the frozen manifest's reserved `__held_evidence_sha256:` error-code marker, so the exact approved manifest binds the held content without changing the existing manifest schema. Admin suppresses this protocol marker from the displayed issue list.

Chunks default to 100 entries and 1 MiB, with total counts and independently verified hashes. These files are prepared offline. The submit command requires `--held-evidence RUN_DIR/held-candidate-evidence.jsonl` whenever the frozen run has candidate-specific package-less entries. It registers every manifest chunk with `finalize:false`, uploads each exact held-evidence artifact through the authenticated `/api/internal/prospect-enrichment/runs/held-evidence` endpoint, then replays the final manifest chunk with `finalize:true`. Finalization fails closed if any candidate-specific hold has no durable matching evidence. Admin's authenticated run reader returns the original candidate and full issue reasons only after verifying the stored digest against the manifest commitment. File-level failures with no candidate identity remain manifest-level source issues and do not invent a candidate record. No generic RPC/database write fallback exists.
...
For a large frozen package set, append `--gzip` and use an output name ending in `.json.gz`. This preserves the exact canonical JSON request bytes inside gzip and reports both decoded-body and compressed-artifact hashes/sizes. Send the file only to the protected comparison-export route with `Content-Encoding: gzip` and the normal authenticated same-origin headers. The route bounds compressed input at 4 MiB and decompressed JSON at 32 MiB; ordinary uncompressed requests retain the 16 MiB cap. Authentication and origin checks still run before body decoding, and gzip never changes package envelopes or their hashes.
