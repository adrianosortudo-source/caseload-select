...
const valueAfter = (argv: readonly string[], flag: string) => {
...
export async function main(argv=process.argv.slice(2)) {
  ...
  const confirm = valueAfter(argv, "--confirm-source-sha256");
  const input = valueAfter(argv, "--input");
  const parsed = input ? JSON.parse(await readFile(input, "utf8")) : RECONCILED_GTA_PROSPECTS;
  const records = Array.isArray(parsed) ? parsed : parsed.records;
  if (!Array.isArray(records)) throw new Error("--input must be an array or a staging manifest with a records array.");
  const plan = await buildGtaProspectImportPlan(records);
  console.log(JSON.stringify({ mode: applyFlag ? "apply" : "dry_run", sourceSha256: plan.sourceSha256, accepted: plan.accepted.length, rejected: plan.rejected }, null, 2));
  if (!applyFlag) return plan.rejected.length ? 1 : 0;
  if (confirm !== plan.sourceSha256) throw new Error("--apply requires matching --confirm-source-sha256.");
  const result = await executeGtaProspectImport({ plan, dryRun: false, operatorAuthorized: true, writer: { apply } });
  return result.state === "applied" ? 0 : 1;
}
