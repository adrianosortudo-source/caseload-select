...
test("exact applied Zarei source key opens only its firm profile and preserves failed read-back evidence", async ({ page }) => {
  ...
  const rows = page.getByRole("row").filter({ hasText: "Synthetic Zarei profile acceptance" });
  ...
  await expect(profile).toContainText("q50-whole-firm-zarei-qualified-2026-09-25-v1");
